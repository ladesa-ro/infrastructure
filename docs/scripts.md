...
Usa o estado ao vivo, não a annotation `kubectl.kubernetes.io/last-applied-configuration`. Já apareceu um caso real neste cluster, o Service `redis-server` (ver [Foundation](foundation.md)), onde o que foi aplicado uma vez diverge do que está rodando hoje. Congelar o `last-applied` reintroduziria essa mudança sem ninguém pedir.
