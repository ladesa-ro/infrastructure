...
flowchart TB
    ...
    Foundation --> Dados[dados: mariadb, minio, adminer]
...
`dados`: `mariadb.yaml`, `minio.yaml` e `adminer.yaml` foram copiados como estão de `/root/dados/k8s/stacks/` no node, sem segredo nenhum, só referenciam Secrets por nome (ver [Dados no cluster](../aprender/dados-no-cluster.md) sobre esse trade-off). `mariadb-env` e `minio-env` chegam via [`InfisicalSecret`](../aprender/infisical.md) (projetos `foundation-mariadb-6s-ji` e `foundation-minio-z-hvh`). Estes três serviços têm mapeamento de porta externo, provavelmente NAT de algum roteador ou proxy fora do cluster: MinIO S3 na 30900, MinIO console na 30901, MariaDB na 30996, Adminer na 30001. Esse mapeamento não está documentado neste repositório; confirmar com quem administra a rede antes de mudar qualquer uma dessas portas. Nenhum desses serviços tem [backup](../aprender/backup-e-disaster-recovery.md) automatizado hoje; se o disco do node morresse, o dado ali seria perdido sem recuperação possível.
