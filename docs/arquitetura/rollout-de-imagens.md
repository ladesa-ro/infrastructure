...
Analisado o código-fonte do handler (`pkg/webhook/ghcr.go`, `pkg/webhook/server.go` do `argoproj-labs/argocd-image-updater`) antes de expor isso publicamente: o payload do GitHub carrega só `{repositório, tag}`, nunca um digest. O handler usa isso só pra saber *qual* imagem recheckar agora; quem resolve o digest de verdade continua sendo uma consulta direta ao registry, no mesmo caminho de reconciliação do polling normal. Não existe campo no payload que vire o digest aplicado, então um payload forjado não tem como injetar imagem nenhuma: na pior hipótese, força uma checagem antecipada de uma imagem que já é real. Ainda assim, o secret HMAC (`X-Hub-Signature-256`, obrigatório por padrão nessa versão, `--webhook-require-secret=true`) segue configurado, com o valor guardado no Infisical (projeto `foundation-image-updater-w0cw`, chave `webhook.ghcr-secret`; nome de chave e de `Secret` (`argocd-image-updater-secret`) fixos pelo próprio controller, não é convenção nossa).

## Estado atual

Implementado em `docs`, `management-service` (`api`/`timetable-worker`) e `web` (2026-08-28), incluindo o webhook. Só `authentication-service` continua sem cobertura. Mesmo mecanismo a replicar (anotações na `Application` + confiar no `namePattern` já genérico do `ImageUpdater` CR); nenhuma mudança de infraestrutura nova precisa entrar. O `authentication-service` tem um `promote.yml` de uma abordagem anterior (troca `tag:` por `digest:` em `values-production.yaml` via commit direto), nunca confirmadamente testado. Com o Image Updater em produção, esse workflow fica obsoleto, não precisa mais ser "consertado", só removido quando o satélite migrar.

### Três bugs reais achados ao vivo depois do deploy (2026-08-28)

O Image Updater ficou instalado, com pod `Running`, mas 100% inerte desde o primeiro deploy, e depois de resolvido ainda escrevia imagem inválida. Três problemas empilhados, achados só investigando ao vivo:

1. **`AppProject ladesa` sem o `ImageUpdater` no whitelist**: `argocd/root/project.yaml` nunca ganhou `argocd-image-updater.argoproj.io/ImageUpdater` no `namespaceResourceWhitelist`. A `Application foundation-image-updater` ficava presa em `OutOfSync`/`Failed` (`resource ... is not permitted in project ladesa`), então o `ImageUpdater` CR nunca sincronizava e o pod nunca aplicava o `watch.namespaces` correto, log preso em `"Watching controller namespace only"`. Corrigido liberando o recurso no whitelist. Esse whitelist chegou a sumir uma segunda vez sem causa raiz confirmada, ver [Pendências](../operacao/pendencias.md#image-updater-estava-instalado-mas-100-inerte-desde-o-pr-5-achado-ao-vivo-em-2026-08-28).
2. **Bug do próprio Argo CD, não do Image Updater**: mesmo com o CR sincronizado e o namespace certo sendo observado, o controller seguia pulando `docs`, `api` e `timetable-worker` com o log `"Image '...' seems not to be live in this application, skipping"`. O Image Updater decide se uma imagem está "live" lendo `Application.status.summary.images`; nesse cluster esse campo está vazio pra toda `Application`, não só as três, confirmado testando outras apps do cluster sem relação nenhuma com o Image Updater. É um bug conhecido e ainda aberto do Argo CD, [argoproj/argo-cd#27861](https://github.com/argoproj/argo-cd/issues/27861), relatado em múltiplas versões e clusters, sem causa raiz identificada upstream ainda. Mitigação documentada pelo próprio mantenedor do Image Updater ([argoproj-labs/argocd-image-updater#791](https://github.com/argoproj-labs/argocd-image-updater/issues/791)): anotação `<alias>.force-update: "true"`, que pula essa checagem de liveness. Efeito colateral aceito: o Image Updater passa a reaplicar o digest resolvido a cada ciclo em vez de só quando muda, sem causar rollout novo se o digest for igual, porque quem decide reiniciar o pod é o Argo CD comparando o manifesto renderizado, não o Image Updater.
3. **`helm.image-tag` apontado pro campo errado, imagem duplicada**: com os dois bugs acima corrigidos, o pod novo passou a subir com `InvalidImageName`, imagem renderizando como `repo:development@development@sha256:...`. Causa: o Image Updater sempre escreve o valor combinado `tag@digest` no parâmetro Helm apontado por `helm.image-tag` (`SetHelmImage`/`GetTagWithDigest()` em `pkg/argocd/argocd.go`), mas a anotação apontava pro campo `application.deployment.image.digest`. O chart `stakater/application` monta a imagem a partir de dois campos separados, `tag` (estático, `development`, de `values-development.yaml`) e `digest` (dinâmico), concatenando os dois (`repo:{{tag}}@{{digest}}`). Escrever o valor já combinado no campo `.digest` duplicava o `tag`. Corrigido apontando `helm.image-tag` pro campo `.tag` em vez de `.digest`, deixando `.digest` sempre vazio: o chart monta `repo:development@sha256:...` a partir de um único campo dinâmico, referência de imagem `nome:tag@digest` válida. Corrigido nos 4 satélites (`docs`, `api`, `timetable-worker`, `web`).
