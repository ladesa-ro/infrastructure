...
### Dois bugs reais achados ao vivo depois do deploy (2026-08-28)

O Image Updater ficou instalado, com pod `Running`, mas 100% inerte desde o primeiro deploy. Dois problemas empilhados, achados só investigando ao vivo:

1. **`AppProject ladesa` sem o `ImageUpdater` no whitelist**: `argocd/root/project.yaml` nunca ganhou `argocd-image-updater.argoproj.io/ImageUpdater` no `namespaceResourceWhitelist`. A `Application foundation-image-updater` ficava presa em `OutOfSync`/`Failed` (`resource ... is not permitted in project ladesa`), então o `ImageUpdater` CR nunca sincronizava e o pod nunca aplicava o `watch.namespaces` correto, log preso em `"Watching controller namespace only"`. Corrigido liberando o recurso no whitelist.
