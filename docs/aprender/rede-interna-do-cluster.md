...
```mermaid
flowchart LR
    Pod1[pod A] -->|veth| Overlay[overlay VXLAN]
    Pod2[pod B] -->|veth| Overlay
    Overlay -->|encapsulado em UDP| Fisica[rede física do node]
    Fisica -->|desencapsula| Pod3[pod em outro node]
```
...
sequenceDiagram
    ...
    participant CoreDNS
    participant SvcB as Service B (ClusterIP)
    participant PodB as pod B (recriado, IP novo)

    PodA->>CoreDNS: resolve "servico-b"
    CoreDNS-->>PodA: ClusterIP fixo do Service
    PodA->>SvcB: requisição pro ClusterIP
    SvcB->>PodB: encaminha pro pod atual, IP que for
```
...
```mermaid
flowchart LR
    Ext[tráfego externo] --> Node[IP do próprio node]
    Node --> Svclb[pod svclb-*]
    Svclb -->|iptables| CIP[ClusterIP real]
    CIP --> PodX[pod de destino]
```
