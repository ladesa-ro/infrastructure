...
```mermaid
flowchart LR
    Dev[quem desenvolve] -->|pede um banco, um ambiente, um namespace| IDP[Internal Developer Platform]
    IDP --> Ansible[Ansible]
    IDP --> ArgoCD[Argo CD]
    IDP --> K8s[Kubernetes]
    Dev -.->|nunca precisa abrir| Ansible
```
...
```mermaid
flowchart TB
    subgraph Portal["Portal (Backstage, Port)"]
        Catalogo[catálogo visual de serviços e templates]
    end
    subgraph API["Camada de API (Kratix, Crossplane)"]
        Padrao[padroniza como infraestrutura é criada]
    end
    Catalogo -->|combinação comum| Padrao
```
