...
- [ ] Senha do [Ansible Vault](../aprender/ansible.md#ansible-vault) gerada em `/root/infrastructure-vault-pass` (*node, via SSH*): [configurar e validar](bootstrap.md#2-gerar-a-senha-do-ansible-vault)
- [ ] [Deploy key](../aprender/ssh.md#chave-pessoal-vs-deploy-key) SSH do `infrastructure` gerada (*node, via SSH*): [configurar e validar](bootstrap.md#3-gerar-e-registrar-a-deploy-key-ssh-do-infrastructure)
...
- [ ] [Gate de drift zero](../arquitetura/gate-de-drift-zero.md): `argocd app diff --core` vazio em **todas** as Applications, não só nas óbvias (*node, via SSH*): [configurar e validar](bootstrap.md#7-gate-de-drift-zero)
