...
- [ ] **`argocd/foundation/rabbitmq/deployment.yaml`**: Deployment simples, uma réplica só, sem HA de verdade. O [RabbitMQ Cluster Operator](https://www.rabbitmq.com/kubernetes/operator/operator-overview) é oficial (mantido pela Broadcom/VMware), padrão de fato pra RabbitMQ em Kubernetes hoje, cuida de peer discovery, rolling upgrade, quorum queues. Achado urgente em 2026-08-21: a imagem `docker.io/bitnami/rabbitmq:3.12` **não existe mais** (`docker pull` retorna `not found`, confirmado ao vivo), o repositório `docker.io/bitnami` foi descontinuado e apagado em 2025 (Broadcom moveu o catálogo pra `bitnamisecure`, versões antigas foram pra `bitnamilegacy`, congelado, sem mais patch de segurança). O pod de produção só segue de pé porque tem a imagem em cache local no node (`imagePullPolicy: IfNotPresent`); qualquer evento que limpe esse cache (reboot, GC de disco, `kubectl delete pod`) deixaria o RabbitMQ sem conseguir subir de novo. Mitigado de imediato trocando pra `docker.io/bitnamilegacy/rabbitmq:3.12` (pinada por digest, ainda puxável, mesmo conteúdo), mas isso não substitui a migração completa: o legacy não recebe mais atualização nenhuma.
